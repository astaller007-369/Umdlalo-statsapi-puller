...
number of seasons (1-5, counting back from the current season). Output is
one CSV per league (each with all its chosen seasons and unplayed fixtures
in it, nothing else), delivered together as a single ZIP download.
...
RESUMING AFTER A NETWORK PROBLEM / INTERRUPTION
    The same SQLite file also keeps a match_progress table, keyed by the
    API's own match_id, recording every finished match's stats the moment
    that one match's /stats call succeeds - not just once the whole run
    finishes. If the connection drops, the page is refreshed, or the app
    restarts partway through a big multi-league fetch, simply click "Fetch
    data" again: every match already recorded is reused with no API call,
    and only the matches that hadn't been fetched yet get requested. The
    (cheap) match list itself is always re-fetched fresh, so a fixture that
    has since kicked off and finished is picked up correctly on the resumed
    run.

RATE LIMITS
    TheStatsAPI plans cap requests/minute (Starter 30, Growth 60, Scale
    higher; some custom/unlimited plans allow far more). Set "Max requests /
    minute" in the sidebar's Advanced section to match your plan - it goes
    up to 2000, so an unlimited-tier key isn't stuck at a low default. The
    app enforces whatever you set client-side and auto-retries on 429s. A full multi-league, multi-season run can be thousands of
...
# ----------------------------------------------------------------------------
# 4b. RESUME-ON-INTERRUPTION SUPPORT
# ----------------------------------------------------------------------------
# A separate table, keyed by the API's own match_id (stable and unique), that
# records the stats of every *successfully* fetched finished match as soon as
# that one match's /stats call comes back - not just at the end of the run.
# If the app is interrupted (network drop, page refresh, server restart)
# partway through a big multi-league fetch, the next click of "Fetch data"
# checks this table before making each /stats call: anything already
# recorded is reused for free, so already-completed work is never re-fetched,
# and only what's genuinely still missing gets requested from the API.
# (The match list itself is always re-fetched fresh and is cheap - that's
# also what lets a scheduled match that has since finished pick up its result.)

MATCH_PROGRESS_COLUMNS = [f"{field}_{side}" for field, _ in METRIC_FIELDS for side in ("home", "away")]


def _ensure_progress_table(conn):
    cols_sql = ", ".join(f'"{c}" TEXT' for c in MATCH_PROGRESS_COLUMNS)
    conn.execute(
        f"""CREATE TABLE IF NOT EXISTS match_progress (
...
_PROGRESS_COLS_QUOTED = ", ".join(f'"{c}"' for c in MATCH_PROGRESS_COLUMNS)
_PROGRESS_COLS_PLACEHOLDERS = ", ".join("?" for _ in MATCH_PROGRESS_COLUMNS)


def get_cached_match_stats(conn, match_id: str) -> dict | None:
    """Returns {field_home/away: value} for a previously-fetched match_id, or None if not cached."""
    cur = conn.execute(
        f"SELECT {_PROGRESS_COLS_QUOTED} FROM match_progress WHERE match_id = ?",
        (match_id,),
    )
    r = cur.fetchone()
    if r is None:
        return None
    return dict(zip(MATCH_PROGRESS_COLUMNS, r))


def save_match_stats_progress(conn, match_id: str, row: dict):
    """Persist one match's fetched stats immediately, so it survives an interruption."""
    values = [row.get(c) for c in MATCH_PROGRESS_COLUMNS]
    conn.execute(
        f"""INSERT OR REPLACE INTO match_progress (match_id, fetched_at, {_PROGRESS_COLS_QUOTED})
            VALUES (?, ?, {_PROGRESS_COLS_PLACEHOLDERS})""",
        [match_id, datetime.now().isoformat()] + values,
    )
    conn.commit()


def progress_cache_count() -> int:
    if not DB_PATH.exists():
        return 0
    conn = _connect()
    try:
        cur = conn.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='match_progress'")
        if cur.fetchone()[0] == 0:
            return 0
        return conn.execute("SELECT COUNT(*) FROM match_progress").fetchone()[0]
    finally:
        conn.close()
...
with st.sidebar.expander("Advanced"):
    requests_per_minute = st.number_input(
        "Max requests / minute", min_value=5, max_value=2000, value=55, step=5,
        help="Match this to your TheStatsAPI plan's rate limit (raise it well past 100 if you're on an unlimited/custom-limit plan).",
    )
    max_workers = st.number_input("Parallel stats requests", min_value=1, max_value=50, value=5, step=1)

run_clicked = st.sidebar.button("Fetch data", type="primary", use_container_width=True)

st.sidebar.markdown("---")
st.sidebar.caption(
    f"Local stat cache holds {cache_row_count()} match rows "
    f"({progress_cache_count()} finished-match stats fetches saved for resuming)."
)
...
st.write(
    ...
    "season, into one CSV per league (a league's file holds every chosen season plus its still-to-come "
    "fixtures, and nothing from any other league). Requesting several leagues at once gets you that many "
    "CSVs, packaged together as one ZIP download."
)

if "result_by_league" not in st.session_state:
    st.session_state.result_by_league = None
...
if run_clicked:
    ...
    resumed_count = 0
    freshly_fetched_count = 0

    # One connection, opened for the whole run, used to check for and record
    # already-fetched match stats so an interruption can be resumed later
    # instead of re-fetching everything from scratch.
    progress_conn = _connect()
    _ensure_progress_table(progress_conn)
            ...
            for season in chosen_seasons:
                ...
                for match in matches:
                    ...
                    if match.get("status") == "finished":
                        match_id = match["id"]
                        cached = get_cached_match_stats(progress_conn, match_id)
                        if cached is not None:
                            # Already fetched in a previous (possibly interrupted) run - reuse it,
                            # no API call needed.
                            row.update(cached)
                            resumed_count += 1
                        else:
                            pending_stats.append((row, match_id))

            if pending_stats:
                status_box.info(
                    f"[{league_idx + 1}/{total_leagues}] {league_display}: fetching stats for "
                    f"{len(pending_stats)} finished matches not already cached..."
                )
                with ThreadPoolExecutor(max_workers=int(max_workers)) as pool:
                    future_to_row = {pool.submit(client.get_match_stats, match_id): (row, match_id) for row, match_id in pending_stats}
                    for future in as_completed(future_to_row):
                        row, match_id = future_to_row[future]
                        try:
                            payload = future.result()
                            apply_stats_to_row(row, payload)
                            # Save this one match's result immediately, so if the run gets cut off
                            # right after this, the work already done isn't lost.
                            save_match_stats_progress(progress_conn, match_id, row)
                            freshly_fetched_count += 1
                        except ApiError as e:
                            errors.append(f"{league_display}: stats for match {match_id} failed ({e.code}) — will retry on next run")
    ...
    if resumed_count:
        st.info(f"Reused {resumed_count} match stats already fetched in a previous run (resumed, not re-downloaded).")
    ...
    # One league = one CSV. Split the combined result back out by the "competition"
    # column (country + league name), which is unique per requested league.
    result_by_league = {
        competition: sub_df.reset_index(drop=True)
        for competition, sub_df in df.groupby("competition", sort=False)
    }

    st.session_state.result_by_league = result_by_league
    status_box.empty()
    progress.empty()
    ...
    st.success(f"Done. {len(df)} match rows across {len(result_by_league)} league(s) — {len(result_by_league)} CSV file(s) ready below.")
...
if st.session_state.get("result_by_league"):
    result_by_league = st.session_state.result_by_league
    league_names = list(result_by_league.keys())

    st.subheader("Preview")
    preview_choice = st.selectbox("League", options=league_names)
    st.dataframe(result_by_league[preview_choice], use_container_width=True)

    # Build one CSV per league, zip them together for a single download.
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    zip_buf = io.BytesIO()
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for competition, league_df in result_by_league.items():
            csv_bytes = league_df.to_csv(index=False).encode("utf-8")
            zf.writestr(f"{_safe_filename(competition)}.csv", csv_bytes)

    st.download_button(
        f"Download all {len(result_by_league)} leagues (ZIP of {len(result_by_league)} CSV files)",
        data=zip_buf.getvalue(),
        file_name=f"thestatsapi_export_{timestamp}.zip",
        mime="application/zip",
        type="primary",
    )

    with st.expander("Download individual league CSVs instead"):
        for competition, league_df in result_by_league.items():
            csv_bytes = league_df.to_csv(index=False).encode("utf-8")
            st.download_button(
                f"{competition} ({len(league_df)} rows)",
                data=csv_bytes,
                file_name=f"{_safe_filename(competition)}.csv",
                mime="text/csv",
                key=f"dl_{_safe_filename(competition)}",
            )
